...
async def login():
    ...
    if status:
        token = jwt.encode({"user_id": id, "exp": datetime.now(tz=timezone.utc) + timedelta(minutes=5)},
                           algorithm=alg,
                           key=secret)
        refresh_token = jwt.encode({"user_id": id, "exp": datetime.now(tz=timezone.utc) + timedelta(hours=1)},
                                   algorithm=alg,
                                   key=secret)
...
async def refresh():
    try:
        ...
        if refresh_token:
            ...
            token = jwt.encode({"user_id": payload["user_id"], "exp": datetime.now(tz=timezone.utc) + timedelta(minutes=5)},
                               algorithm=alg,
                               key=secret)
            if payload["exp"] - time.mktime(datetime.now(tz=timezone.utc).timetuple()) < 1000 * 3600:
                refresh_token = jwt.encode({"user_id": payload["user_id"],
